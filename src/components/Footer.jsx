...
const Footer = () => {
  ...
  return (
    <footer className="mt-auto border-t border-white/5 bg-(--surface-bg) text-(--text-color)">
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-7 px-4 py-8 sm:px-6 sm:py-10 lg:grid-cols-[1.1fr_1.9fr] lg:gap-10 lg:px-12 xl:px-20">
        ...
        <nav aria-label="Footer mobile links" className="sm:hidden">
          <ul className="flex flex-wrap items-center justify-center gap-x-5 gap-y-3 text-sm font-semibold">
            {mobileLinks.map((link) => (
              <li key={link.href}>
                <a
                  className="text-(--text-color) transition hover:text-(--primary-color)"
                  href={link.href}
                >
                  {link.label}
                </a>
        ...
        <div className="hidden grid-cols-1 gap-6 sm:grid sm:grid-cols-3 sm:gap-8">
          {footerLinks.map((group) => (
            <nav key={group.title} aria-label={group.title}>
              <h2 className="text-sm font-black uppercase tracking-[0.16em] text-(--title-color)">
                {group.title}
              </h2>
      ...
      <div className="border-t border-white/5 px-4 py-5 text-sm font-medium sm:px-6 lg:px-12 xl:px-20">
        <div className="mx-auto flex max-w-7xl flex-col-reverse gap-2 text-center sm:flex-row sm:items-center sm:justify-between sm:text-left">
          <p>
            ...
            {siteSettings?.businessName || "KD Studios"}. All rights reserved.
          </p>
          <a
            className="text-(--primary-color) transition hover:text-(--accent-color)"
            href={`tel:${siteSettings?.phone || "+233595363184"}`}
          >
            {siteSettings?.phone || "+233 59 536 3184"}
          </a>
        </div>
      </div>
    </footer>
